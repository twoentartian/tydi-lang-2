...
#[test]
fn casper_bit_in_stream_not_exported() {
    let project = Project::new(format!("sample_project"));
    {
        let mut project_write = project.write().unwrap();
        ...
        let src_pack0 = String::from(r#"
            ...
            streamlet bypass_s {
                # this is port_in #
                port_in: bitStream in;
            
                bitStream2 = Stream(d);
            
                # this is port_out #
                port_out: bitStream2 out;
            }
    ...
    project.read().unwrap().evaluate_target(format!("bypass_i"), format!("aliasTest")).expect("fail to evaluate");

    let code_structure = project.read().unwrap().get_pretty_json();
    std::fs::write("./code_structure.json", &code_structure).unwrap();

    let json_output = generate_json_representation_from_tydi_project(project.clone(), format!("bypass_i"), format!("aliasTest")).expect("fail to generate json");
    std::fs::write("./json_output.json", &json_output).unwrap();
    println!("{}", json_output);
}
