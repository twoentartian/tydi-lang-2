...
pub fn parse_Implementation(src: Pair<Rule>, scope: Arc<RwLock<Scope>>) -> Result<Arc<RwLock<Variable>>, TydiLangError> {
    ...
    for element in src.clone().into_inner().into_iter() {
        ...
        match rule {
            ...
            Rule::Exp => {
                ...
                streamlet_exp_code_location = CodeLocation::new_from_pest_rule(&element);
            }
            Rule::ATTRIBUTE => {
            ...
            Rule::Scope_WithoutBracket => {
                ...
                parse_file::parse_Scope_WithoutBracket(element, output_implementation.read().unwrap().get_scope())?;
            }
            _ => unreachable!()
        }
    }
    {
        ...
        output_implementation_write.set_code_location(CodeLocation::new_from_pest_rule(&src));
    ...
    let output_implementation_var = Variable::new_implementation(name.clone(), output_implementation);
    {
        ...
        output_implementation_var_write.set_code_location(CodeLocation::new_from_pest_rule(&src));
    }
...
#[allow(non_snake_case)]
pub fn parse_Instance(src: Pair<Rule>, scope: Arc<RwLock<Scope>>) -> Result<Arc<RwLock<Variable>>, TydiLangError> {
    let mut document = None;
    let mut name = generate_init_value();
    let mut attributes = vec![];
    ...
    for element in src.clone().into_inner().into_iter() {
        ...
        match rule {
            ...
            Rule::Exp => {
                ...
                implementation_exp_location = CodeLocation::new_from_pest_rule(&element);
            }
            Rule::ArraySizeIndicator => {
                array_size_indicator = parse_ArraySizeIndicator(element.clone(), scope.clone())?;
                if array_size_indicator.is_none() {
                    return Err(TydiLangError::new(format!("unknown array size indicator is not allowed for declaring instance"), CodeLocation::new_from_pest_rule(&element)));
                }
            }
            Rule::ATTRIBUTE => {
                let attr = parse_miscellaneous::parse_ATTRIBUTE(element, scope.clone())?;
    ...
    let output_instance = Instance::new(name.clone(), implementation_exp.clone());
    {
        ...
        output_instance_write.set_code_location(CodeLocation::new_from_pest_rule(&src));
    }
    ...
    let output_instance_var = Variable::new_instance(name.clone(), output_instance);
    {
        ...
        output_instance_var_write.set_code_location(CodeLocation::new_from_pest_rule(&src));
...
#[allow(non_snake_case)]
pub fn parse_Net(src: Pair<Rule>, scope: Arc<RwLock<Scope>>) -> Result<Arc<RwLock<Variable>>, TydiLangError> {
    let mut document = None;
    ...
    for element in src.clone().into_inner().into_iter() {
        ...
        match rule {
            ...
            Rule::Exp => {
                if exp_index == 0 {
                    source_var = Variable::new_with_type_indication(generate_built_in_variable_name_from_span(&element), Some(element.as_str().to_string()), TypeIndication::AnyPort);
                    {
                        ...
                        source_var_write.set_code_location(CodeLocation::new_from_pest_rule(&element));
                    }
                } else if exp_index == 1 {
                    sink_var = Variable::new_with_type_indication(generate_built_in_variable_name_from_span(&element), Some(element.as_str().to_string()), TypeIndication::AnyPort);
                    {
                        ...
                        sink_var_write.set_code_location(CodeLocation::new_from_pest_rule(&element));
                    }
                } else {
                    unreachable!();
                }
            ...
            Rule::NetName => {
                let parsed_name = parse_Netname(element, scope.clone())?;
                match parsed_name {
    ...
    let output_net = Net::new(name.clone());
    {
        ...
        output_net_write.set_code_location(CodeLocation::new_from_pest_rule(&src));
    }
    let output_net_var = Variable::new_net(name.clone(), output_net);
    {
        let mut output_net_var_write = output_net_var.write().unwrap();
        output_net_var_write.set_code_location(CodeLocation::new_from_pest_rule(&src));
    }
...
#[allow(non_snake_case)]
pub fn parse_Netname(src: Pair<Rule>, scope: Arc<RwLock<Scope>>) -> Result<Option<Arc<RwLock<Variable>>>, TydiLangError> {
    for element in src.clone().into_inner().into_iter() {
        let rule = element.as_rule();
        match rule {
            Rule::Exp => {
                let output_var = Variable::new_with_type_indication(generate_built_in_variable_name_from_span(&element), Some(element.as_str().to_string()), TypeIndication::String);
                {
                    ...
                    output_var_write.set_code_location(CodeLocation::new_from_pest_rule(&element));
                }
