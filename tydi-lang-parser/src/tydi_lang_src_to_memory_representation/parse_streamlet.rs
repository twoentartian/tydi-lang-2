...
#[allow(non_snake_case)]
pub fn parse_Port(src: Pair<Rule>, scope: Arc<RwLock<Scope>>) -> Result<Arc<RwLock<Variable>>, TydiLangError> {
    let mut document = None;
    let mut name = generate_init_value();
    let mut attributes = vec![];
        ...
        match rule {
            ...
            Rule::Exp => {
                port_logical_type_exp = Variable::new_with_type_indication(generate_built_in_variable_name_from_span(&element), Some(element.as_str().to_string()), TypeIndication::AnyLogicType);
                {
                    ...
                    port_logical_type_exp_write.set_code_location(CodeLocation::new_from_pest_rule(&element));
                }
            }
            Rule::PortDirection => {
                port_driection = parse_PortDirection(element, scope.clone())?;
            }
            Rule::ArraySizeIndicator => {
                array_size_indicator = parse_ArraySizeIndicator(element, scope.clone())?;
            }
            Rule::PortTimeDomain => {
                port_time_domain_exp = parse_PortTimeDomain(element.clone(), scope.clone())?;
                {
                    if port_time_domain_exp.is_some() {
                        ...
                        port_time_domain_exp_write.set_code_location(CodeLocation::new_from_pest_rule(&element));
                    }
                }
            }
            Rule::ATTRIBUTE => {
    ...
    let output_port = Port::new(generate_built_in_variable_name_from_span(&src), port_driection, port_logical_type_exp);
    {
        ...
        output_port_write.set_code_location(CodeLocation::new_from_pest_rule(&src));
...
#[allow(non_snake_case)]
pub fn parse_PortTimeDomain(src: Pair<Rule>, scope: Arc<RwLock<Scope>>) -> Result<Option<Arc<RwLock<Variable>>>, TydiLangError> {
    for element in src.clone().into_inner().into_iter() {
        let rule = element.as_rule();
        match rule {
            Rule::Exp => {
...
#[allow(non_snake_case)]
pub fn parse_PortDirection(src: Pair<Rule>, scope: Arc<RwLock<Scope>>) -> Result<PortDirection, TydiLangError> {
    for element in src.clone().into_inner().into_iter() {
        let rule = element.as_rule();
        match rule {
