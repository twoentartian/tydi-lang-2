...
pub fn parse_LogicalBit(src: Pair<Rule>, scope: Arc<RwLock<Scope>>) -> Result<Arc<RwLock<Variable>>, TydiLangError> {
    ...
    let output_logic_bit = LogicBit::new(generate_name::generate_built_in_variable_name_from_span(&src), Some(bit_exp));
    {
        ...
        output_logic_bit_write.set_code_location(CodeLocation::new_from_pest_rule(&src));
    }
...
#[allow(non_snake_case)]
pub fn parse_StreamProperty(src: Pair<Rule>, scope: Arc<RwLock<Scope>>) -> Result<Arc<RwLock<Variable>>, TydiLangError> {
    ...
    if stream_property.is_err() {
        return Err(TydiLangError::new(format!("unknown stream property: {}", name), CodeLocation::new_from_pest_rule(&src)));
    }
    ...
    let output_var = Variable::new(stream_property.get_full_name().to_string(), Some(exp));
    {
        ...
        output_var_write.set_code_location(CodeLocation::new_from_pest_rule(&src));
    }
    return Ok(output_var);
}

#[allow(non_snake_case)]
pub fn parse_LogicalStream(src: Pair<Rule>, scope: Arc<RwLock<Scope>>) -> Result<Arc<RwLock<Variable>>, TydiLangError> {
    ...
    for element in src.clone().into_inner().into_iter() {
        ...
        match rule {
            Rule::Exp => {
                ...
                stream_type_location = CodeLocation::new_from_pest_rule(&src);
            }
            Rule::StreamProperty => {
                let property = parse_StreamProperty(element, scope.clone())?;
    ...
    // add stream property
    {
        ...
        output_logic_stream_write.set_code_location(CodeLocation::new_from_pest_rule(&src));
        for stream_property in stream_properties {
            let result = output_logic_stream_write.apply_property_var(stream_property);
            if result.is_err() {
                return Err(TydiLangError::new(result.err().unwrap(), CodeLocation::new_from_pest_rule(&src)));
            }
        }
    }
    // set stream variable
    let logic_stream_var = Variable::new_logic_type(logic_stream_var_name, Arc::new(RwLock::new(LogicType::LogicStreamType(output_logic_stream))));
    {
        ...
        logic_stream_var_write.set_code_location(CodeLocation::new_from_pest_rule(&src));
    }
    //write var to scope
    {
        let mut scope_write = scope.write().unwrap();
